...
function CurrentProjectID() {
	...
	return "{67DBC02E-FD20-415A-8CF5-94DD09C14F7A}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionUserID($rs->fields('staff_id')); // Load User ID
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('staff_id'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
	...
	function User_Validated(&$rs) {
		...
		$_SESSION['Staff_Name'] = $rs['first_name'] ." ". $rs['last_name'] ." ". $rs['mobile'];
