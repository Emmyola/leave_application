...
$leave_form_edit = NULL; // Initialize page object first

class cleave_form_edit extends cleave_form {
	...
	// Page object name
	var $PageObjName = 'leave_form_edit';
	...
	function RenderRow() {
		...
		$this->last_updated_date->ViewCustomAttributes = "";
			...
			$this->replacement_assign_staff->LookupFilters = array("dx1" => '`first_name`', "dx2" => '`last_name`', "dx3" => '`mobile`');
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			if (!$GLOBALS["leave_form"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["user_profile"]->AddUserIDFilter($sWhereWrk);
			$this->Lookup_Selecting($this->replacement_assign_staff, $sWhereWrk); // Call Lookup Selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			$rswrk = Conn()->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// leave_id
			$this->leave_id->SetDbValueDef($rsnew, $this->leave_id->CurrentValue, "", $this->leave_id->ReadOnly);
			...
			// time
			$this->time->SetDbValueDef($rsnew, $this->time->CurrentValue, ew_CurrentTime(), $this->time->ReadOnly);

			// staff_id
			$this->staff_id->SetDbValueDef($rsnew, $this->staff_id->CurrentValue, "", $this->staff_id->ReadOnly);

			// staff_name
			$this->staff_name->SetDbValueDef($rsnew, $this->staff_name->CurrentValue, "", $this->staff_name->ReadOnly);

			// company
			$this->company->SetDbValueDef($rsnew, $this->company->CurrentValue, "", $this->company->ReadOnly);

			// department
			$this->department->SetDbValueDef($rsnew, $this->department->CurrentValue, 0, $this->department->ReadOnly);

			// leave_type
			$this->leave_type->SetDbValueDef($rsnew, $this->leave_type->CurrentValue, 0, $this->leave_type->ReadOnly);

			// start_date
			$this->start_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->start_date->CurrentValue, 14), ew_CurrentDate(), $this->start_date->ReadOnly);

			// end_date
			$this->end_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->end_date->CurrentValue, 14), ew_CurrentDate(), $this->end_date->ReadOnly);

			// no_of_days
			$this->no_of_days->SetDbValueDef($rsnew, $this->no_of_days->CurrentValue, 0, $this->no_of_days->ReadOnly);

			// resumption_date
			$this->resumption_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->resumption_date->CurrentValue, 14), ew_CurrentDate(), $this->resumption_date->ReadOnly);

			// replacement_assign_staff
			$this->replacement_assign_staff->SetDbValueDef($rsnew, $this->replacement_assign_staff->CurrentValue, "", $this->replacement_assign_staff->ReadOnly);

			// purpose_of_leave
			$this->purpose_of_leave->SetDbValueDef($rsnew, $this->purpose_of_leave->CurrentValue, "", $this->purpose_of_leave->ReadOnly);

			// status
			$this->status->SetDbValueDef($rsnew, $this->status->CurrentValue, 0, $this->status->ReadOnly);

			// initiator_action
			$this->initiator_action->SetDbValueDef($rsnew, $this->initiator_action->CurrentValue, 0, $this->initiator_action->ReadOnly);

			// initiator_comments
			$this->initiator_comments->SetDbValueDef($rsnew, $this->initiator_comments->CurrentValue, "", $this->initiator_comments->ReadOnly);

			// verified_staff
			$this->verified_staff->SetDbValueDef($rsnew, $this->verified_staff->CurrentValue, NULL, $this->verified_staff->ReadOnly);

			// verified_replacement_staff
			$this->verified_replacement_staff->SetDbValueDef($rsnew, $this->verified_replacement_staff->CurrentValue, NULL, $this->verified_replacement_staff->ReadOnly);

			// recommender_action
			$this->recommender_action->SetDbValueDef($rsnew, $this->recommender_action->CurrentValue, NULL, $this->recommender_action->ReadOnly);

			// recommender_comments
			$this->recommender_comments->SetDbValueDef($rsnew, $this->recommender_comments->CurrentValue, NULL, $this->recommender_comments->ReadOnly);

			// approver_action
			$this->approver_action->SetDbValueDef($rsnew, $this->approver_action->CurrentValue, NULL, $this->approver_action->ReadOnly);

			// approver_comments
			$this->approver_comments->SetDbValueDef($rsnew, $this->approver_comments->CurrentValue, NULL, $this->approver_comments->ReadOnly);

			// last_updated_date
			$this->last_updated_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->last_updated_date->CurrentValue, 7), NULL, $this->last_updated_date->ReadOnly);
	...
	function GetRowHash(&$rs) {
		if (!$rs)
			return "";
		$sHash = "";
		$sHash .= ew_GetFldHash($rs->fields('leave_id')); // leave_id
		$sHash .= ew_GetFldHash($rs->fields('date_created')); // date_created
		$sHash .= ew_GetFldHash($rs->fields('time')); // time
		$sHash .= ew_GetFldHash($rs->fields('staff_id')); // staff_id
		$sHash .= ew_GetFldHash($rs->fields('staff_name')); // staff_name
		$sHash .= ew_GetFldHash($rs->fields('company')); // company
		$sHash .= ew_GetFldHash($rs->fields('department')); // department
		$sHash .= ew_GetFldHash($rs->fields('leave_type')); // leave_type
		$sHash .= ew_GetFldHash($rs->fields('start_date')); // start_date
		$sHash .= ew_GetFldHash($rs->fields('end_date')); // end_date
		$sHash .= ew_GetFldHash($rs->fields('no_of_days')); // no_of_days
		$sHash .= ew_GetFldHash($rs->fields('resumption_date')); // resumption_date
		$sHash .= ew_GetFldHash($rs->fields('replacement_assign_staff')); // replacement_assign_staff
		$sHash .= ew_GetFldHash($rs->fields('purpose_of_leave')); // purpose_of_leave
		$sHash .= ew_GetFldHash($rs->fields('status')); // status
		$sHash .= ew_GetFldHash($rs->fields('initiator_action')); // initiator_action
		$sHash .= ew_GetFldHash($rs->fields('initiator_comments')); // initiator_comments
		$sHash .= ew_GetFldHash($rs->fields('verified_staff')); // verified_staff
		$sHash .= ew_GetFldHash($rs->fields('verified_replacement_staff')); // verified_replacement_staff
		$sHash .= ew_GetFldHash($rs->fields('recommender_action')); // recommender_action
		$sHash .= ew_GetFldHash($rs->fields('recommender_comments')); // recommender_comments
		$sHash .= ew_GetFldHash($rs->fields('approver_action')); // approver_action
		$sHash .= ew_GetFldHash($rs->fields('approver_comments')); // approver_comments
		$sHash .= ew_GetFldHash($rs->fields('last_updated_date')); // last_updated_date
		return md5($sHash);
	}
		...
		// Return error message in CustomError
		   $rs = $this->GetFieldValues("FormValue"); // Get the form values as array
			...
			if(CurrentUserLevel() == 2 && $this->status->CurrentValue == 3 && ($rs["recommender_action"] == '' || $rs["recommender_comments"] == '')){
				$CustomError = "Recommender Action / Comments Field Can not be Empty.";
				return FALSE;
			}

			///recommender validation
			if(CurrentUserLevel() == 3 && $this->status->CurrentValue == 4 && ($rs["approver_action"] == '' || $rs["approver_comments"] == '')){
				$CustomError = "Appprover Action / Comments Field Can not be Empty.";
				return FALSE;
			}
		return TRUE;
	}
...
// Create page object
if (!isset($leave_form_edit)) $leave_form_edit = new cleave_form_edit();

// Page init
$leave_form_edit->Page_Init();

// Page main
$leave_form_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$leave_form_edit->Page_Render();
?>
...
var CurrentForm = fleave_formedit = new ew_Form("fleave_formedit", "edit");

// Validate form
fleave_formedit.Validate = function() {
...
fleave_formedit.Form_CustomValidate = 
...
fleave_formedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
fleave_formedit.MultiPage = new ew_MultiPage("fleave_formedit");

// Dynamic selection lists
fleave_formedit.Lists["x_company"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_department","x_replacement_assign_staff"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"company"};
fleave_formedit.Lists["x_company"].Data = "<?php echo $leave_form_edit->company->LookupFilterQuery(FALSE, "edit") ?>";
fleave_formedit.Lists["x_department"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":["x_company"],"ChildFields":[],"FilterFields":["x_company_id"],"Options":[],"Template":"","LinkTable":"department"};
fleave_formedit.Lists["x_department"].Data = "<?php echo $leave_form_edit->department->LookupFilterQuery(FALSE, "edit") ?>";
fleave_formedit.Lists["x_leave_type"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"leave_type"};
fleave_formedit.Lists["x_leave_type"].Data = "<?php echo $leave_form_edit->leave_type->LookupFilterQuery(FALSE, "edit") ?>";
fleave_formedit.Lists["x_replacement_assign_staff"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_first_name","x_last_name","x_mobile",""],"ParentFields":["x_company"],"ChildFields":[],"FilterFields":["x_company"],"Options":[],"Template":"","LinkTable":"user_profile"};
fleave_formedit.Lists["x_replacement_assign_staff"].Data = "<?php echo $leave_form_edit->replacement_assign_staff->LookupFilterQuery(FALSE, "edit") ?>";
fleave_formedit.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status"};
fleave_formedit.Lists["x_status"].Data = "<?php echo $leave_form_edit->status->LookupFilterQuery(FALSE, "edit") ?>";
fleave_formedit.AutoSuggests["x_status"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $leave_form_edit->status->LookupFilterQuery(TRUE, "edit"))) ?>;
fleave_formedit.Lists["x_initiator_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fleave_formedit.Lists["x_initiator_action"].Options = <?php echo json_encode($leave_form_edit->initiator_action->Options()) ?>;
fleave_formedit.Lists["x_verified_staff"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fleave_formedit.Lists["x_verified_staff"].Options = <?php echo json_encode($leave_form_edit->verified_staff->Options()) ?>;
fleave_formedit.Lists["x_verified_replacement_staff"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fleave_formedit.Lists["x_verified_replacement_staff"].Options = <?php echo json_encode($leave_form_edit->verified_replacement_staff->Options()) ?>;
fleave_formedit.Lists["x_recommender_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fleave_formedit.Lists["x_recommender_action"].Options = <?php echo json_encode($leave_form_edit->recommender_action->Options()) ?>;
fleave_formedit.Lists["x_approver_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fleave_formedit.Lists["x_approver_action"].Options = <?php echo json_encode($leave_form_edit->approver_action->Options()) ?>;
...
$(document).ready(function(){
	...
	var replacement;
	var recommender
	$("#x_recommender_action_1").attr('disabled',true);
	/

	// ==================Verified Staff==========================
	$("input[name='x_verified_staff']").click(function(){
		leave = $("input[name='x_verified_staff']:checked").val();
		if((leave == 2) && (replacement == 2)){
			$("#x_recommender_action_1").attr('disabled',false);
		}else{
		$("#x_recommender_action_1").attr('disabled',true);

		//$("input[name='#x_recommender_action_1']").attr('checked', false);
		//$("#x_recommender_action_1").removeAttr('checked');

	}
	})

	// ============Verified Replacement================
	$("input[name='x_verified_replacement_staff']").click(function(){
		replacement = $("input[name='x_verified_replacement_staff']:checked").val();
		if((leave == 2) && (replacement == 2)){
			$("#x_recommender_action_1").attr('disabled',false);
		}else{
			$("#x_recommender_action_1").attr('disabled',true);
	...
	// ======================Recommender Action=========================
	$("input[name='x_recommender_action']").click(function(){
		recommender = $("input[name='x_recommender_action']:checked").val();
		if(recommender == 2){
			$("#x_verified_staff_0").attr('disabled',true);
			$("#x_verified_replacement_staff_0").attr('disabled',true);
		}
		else{
			$("#x_verified_staff_0").attr('disabled',false);
			$("#x_verified_replacement_staff_0").attr('disabled',false);
		}
	})
...
<?php $leave_form_edit->ShowPageHeader(); ?>
<?php
$leave_form_edit->ShowMessage();
?>
<form name="fleave_formedit" id="fleave_formedit" class="<?php echo $leave_form_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($leave_form_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $leave_form_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="leave_form">
<input type="hidden" name="k_hash" id="k_hash" value="<?php echo $leave_form_edit->HashValue ?>">
<?php if ($leave_form->UpdateConflict == "U") { // Record already updated by other user ?>
...
<?php } ?>
<input type="hidden" name="modal" value="<?php echo intval($leave_form_edit->IsModal) ?>">
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="leave_form_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $leave_form_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $leave_form_edit->MultiPages->TabStyle("1") ?>><a href="#tab_leave_form1" data-toggle="tab"><?php echo $leave_form->PageCaption(1) ?></a></li>
		<li<?php echo $leave_form_edit->MultiPages->TabStyle("2") ?>><a href="#tab_leave_form2" data-toggle="tab"><?php echo $leave_form->PageCaption(2) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $leave_form_edit->MultiPages->PageStyle("1") ?>" id="tab_leave_form1"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($leave_form->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_leave_form_id" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->id->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->id->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
<span id="el_leave_form_id">
<span<?php echo $leave_form->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $leave_form->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="leave_form" data-field="x_id" data-page="1" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($leave_form->id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_leave_form_id">
<span<?php echo $leave_form->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $leave_form->id->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="leave_form" data-field="x_id" data-page="1" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($leave_form->id->FormValue) ?>">
<?php } ?>
<?php echo $leave_form->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($leave_form->leave_id->Visible) { // leave_id ?>
	<div id="r_leave_id" class="form-group">
		<label id="elh_leave_form_leave_id" for="x_leave_id" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->leave_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->leave_id->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->date_created->Visible) { // date_created ?>
	<div id="r_date_created" class="form-group">
		<label id="elh_leave_form_date_created" for="x_date_created" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->date_created->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->date_created->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->time->Visible) { // time ?>
	<div id="r_time" class="form-group">
		<label id="elh_leave_form_time" for="x_time" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->time->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->time->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->staff_id->Visible) { // staff_id ?>
	<div id="r_staff_id" class="form-group">
		<label id="elh_leave_form_staff_id" for="x_staff_id" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->staff_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->staff_id->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->staff_name->Visible) { // staff_name ?>
	<div id="r_staff_name" class="form-group">
		<label id="elh_leave_form_staff_name" for="x_staff_name" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->staff_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->staff_name->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->company->Visible) { // company ?>
	<div id="r_company" class="form-group">
		<label id="elh_leave_form_company" for="x_company" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->company->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->company->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->department->Visible) { // department ?>
	<div id="r_department" class="form-group">
		<label id="elh_leave_form_department" for="x_department" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->department->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->department->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->leave_type->Visible) { // leave_type ?>
	<div id="r_leave_type" class="form-group">
		<label id="elh_leave_form_leave_type" for="x_leave_type" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->leave_type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->leave_type->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->start_date->Visible) { // start_date ?>
	<div id="r_start_date" class="form-group">
		<label id="elh_leave_form_start_date" for="x_start_date" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->start_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->start_date->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fleave_formedit", "x_start_date", {"ignoreReadonly":true,"useCurrent":false,"format":14});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<?php if ($leave_form->end_date->Visible) { // end_date ?>
	<div id="r_end_date" class="form-group">
		<label id="elh_leave_form_end_date" for="x_end_date" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->end_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->end_date->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fleave_formedit", "x_end_date", {"ignoreReadonly":true,"useCurrent":false,"format":14});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<?php if ($leave_form->no_of_days->Visible) { // no_of_days ?>
	<div id="r_no_of_days" class="form-group">
		<label id="elh_leave_form_no_of_days" for="x_no_of_days" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->no_of_days->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->no_of_days->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->resumption_date->Visible) { // resumption_date ?>
	<div id="r_resumption_date" class="form-group">
		<label id="elh_leave_form_resumption_date" for="x_resumption_date" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->resumption_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->resumption_date->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
ew_CreateDateTimePicker("fleave_formedit", "x_resumption_date", {"ignoreReadonly":true,"useCurrent":false,"format":14});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<?php if ($leave_form->replacement_assign_staff->Visible) { // replacement_assign_staff ?>
	<div id="r_replacement_assign_staff" class="form-group">
		<label id="elh_leave_form_replacement_assign_staff" for="x_replacement_assign_staff" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->replacement_assign_staff->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->replacement_assign_staff->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->purpose_of_leave->Visible) { // purpose_of_leave ?>
	<div id="r_purpose_of_leave" class="form-group">
		<label id="elh_leave_form_purpose_of_leave" for="x_purpose_of_leave" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->purpose_of_leave->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->purpose_of_leave->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $leave_form_edit->MultiPages->PageStyle("2") ?>" id="tab_leave_form2"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($leave_form->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label id="elh_leave_form_status" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->status->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
fleave_formedit.CreateAutoSuggest({"id":"x_status","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_leave_form_status">
...
<?php if ($leave_form->initiator_action->Visible) { // initiator_action ?>
	<div id="r_initiator_action" class="form-group">
		<label id="elh_leave_form_initiator_action" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->initiator_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->initiator_action->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->initiator_comments->Visible) { // initiator_comments ?>
	<div id="r_initiator_comments" class="form-group">
		<label id="elh_leave_form_initiator_comments" for="x_initiator_comments" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->initiator_comments->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->initiator_comments->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->verified_staff->Visible) { // verified_staff ?>
	<div id="r_verified_staff" class="form-group">
		<label id="elh_leave_form_verified_staff" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->verified_staff->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->verified_staff->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->verified_replacement_staff->Visible) { // verified_replacement_staff ?>
	<div id="r_verified_replacement_staff" class="form-group">
		<label id="elh_leave_form_verified_replacement_staff" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->verified_replacement_staff->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->verified_replacement_staff->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->recommender_action->Visible) { // recommender_action ?>
	<div id="r_recommender_action" class="form-group">
		<label id="elh_leave_form_recommender_action" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->recommender_action->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->recommender_action->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->recommender_comments->Visible) { // recommender_comments ?>
	<div id="r_recommender_comments" class="form-group">
		<label id="elh_leave_form_recommender_comments" for="x_recommender_comments" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->recommender_comments->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->recommender_comments->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->approver_action->Visible) { // approver_action ?>
	<div id="r_approver_action" class="form-group">
		<label id="elh_leave_form_approver_action" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->approver_action->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->approver_action->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->approver_comments->Visible) { // approver_comments ?>
	<div id="r_approver_comments" class="form-group">
		<label id="elh_leave_form_approver_comments" for="x_approver_comments" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->approver_comments->FldCaption() ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->approver_comments->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if ($leave_form->last_updated_date->Visible) { // last_updated_date ?>
	<div id="r_last_updated_date" class="form-group">
		<label id="elh_leave_form_last_updated_date" for="x_last_updated_date" class="<?php echo $leave_form_edit->LeftColumnClass ?>"><?php echo $leave_form->last_updated_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $leave_form_edit->RightColumnClass ?>"><div<?php echo $leave_form->last_updated_date->CellAttributes() ?>>
<?php if ($leave_form->CurrentAction <> "F") { ?>
...
<?php if (!$leave_form_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $leave_form_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<?php if ($leave_form->UpdateConflict == "U") { // Record already updated by other user ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $leave_form_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
fleave_formedit.Init();
</script>
<?php
$leave_form_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$leave_form_edit->Page_Terminate();
?>
