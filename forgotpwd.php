...
class cforgotpwd extends cuser_profile {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["user_profile"])) $GLOBALS["user_profile"] = &$this;
	...
	function Page_Main() {
		...
		$Breadcrumb->Add("forgotpwd", "RequestPwd", ew_CurrentUrl(), "", "", TRUE);
				...
				if ($bValidEmail) {
					if (strtolower($this->Action) == "resetpassword") { // Reset password
						...
						if (strtolower($this->Action) == "confirm") {
							...
							$sActivateLink = ew_FullUrl("", "resetpwd") . "?action=reset";
...
fforgotpwd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
</script>
<?php $forgotpwd->ShowPageHeader(); ?>
<?php
$forgotpwd->ShowMessage();
?>
<form name="fforgotpwd" id="fforgotpwd" class="ewForm ewForgotpwdForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($forgotpwd->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $forgotpwd->Token ?>">
<?php } ?>
<div class="login-box ewForgotpwdBox">
<div class="login-box-body">
<p class="login-box-msg"><?php echo $Language->Phrase("RequestPwd") ?></p>
	<div class="form-group">
		<div><input type="text" name="email" id="email" class="form-control ewControl" value="<?php echo ew_HtmlEncode($forgotpwd->Email) ?>" size="30" maxlength="25" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("UserEmail")) ?>"></div>
	</div>
